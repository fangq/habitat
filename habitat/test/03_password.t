...
# ----------------------------------------------------------------
ok( !HabitatEngine::IsLegacyPasswordHash($h1),
    "bcrypt hash is NOT flagged as legacy" );
ok( !HabitatEngine::IsLegacyPasswordHash(''),
    "empty hash is NOT flagged as legacy (nothing to migrate)" );
ok( !HabitatEngine::IsLegacyPasswordHash(undef),
    "undef hash is NOT flagged as legacy" );

# crypt() classic 13-char DES output is legacy
my $legacy_des = crypt( "hello", "ab" );    # 13 chars on glibc, '$1$' on some platforms
ok( HabitatEngine::IsLegacyPasswordHash($legacy_des),
    "plain crypt() output IS flagged as legacy" )
...
SKIP: {
    ...
    ok( HabitatEngine::VerifyPassword( "hello", $legacy_des ),
        "correct password verifies against legacy crypt() hash" );
    ok( !HabitatEngine::VerifyPassword( "nope", $legacy_des ),
