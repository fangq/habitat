...
like( $tok, qr/\A\d+\|\d+\|[0-9a-f]{64}\z/, "token shape: <uid>|<exp>|<64-hex sig>" );
is( HabitatEngine::VerifySessionToken($tok), 1001, "fresh token verifies and returns the uid" );
...
is( HabitatEngine::VerifySessionToken($tampered), 0, "uid tampering invalidates token" );

# Tampered signature
my $bad_sig = $tok;
substr( $bad_sig, -1 ) = ( substr( $bad_sig, -1 ) eq 'a' ) ? 'b' : 'a';
is( HabitatEngine::VerifySessionToken($bad_sig), 0, "signature tampering invalidates token" );

# Tampered expiry -- changes signed content
my $bad_exp = "$uid|" . ( $exp + 1 ) . "|$sig";
is( HabitatEngine::VerifySessionToken($bad_exp), 0, "expiry tampering invalidates token" );
...
my $tok_short = HabitatEngine::SignSessionToken( 1001, 60 );
{
    ...
    is( HabitatEngine::VerifySessionToken($tok_short), 1001, "token valid 1s before expiry" );
}
{
    HabitatHarness::freeze_time( 1_700_000_000 + 61 );
    is( HabitatEngine::VerifySessionToken($tok_short), 0, "token rejected 1s after expiry" );
}
...
my $tok_old_secret = HabitatEngine::SignSessionToken(1001);
{
    ...
    is( HabitatEngine::VerifySessionToken($tok_old_secret),
        0, "rotating the site secret invalidates pre-rotation tokens" );
}
...
}
{
    ...
    is( HabitatEngine::IsRequestSecure(),
        0, "X-Forwarded-Proto ignored when no trusted proxies configured" );

    local $HabitatEngine::TrustedProxies = '127.0.0.1';
    is( HabitatEngine::IsRequestSecure(),
        1, "X-Forwarded-Proto honored when REMOTE_ADDR is in TrustedProxies" );
}
