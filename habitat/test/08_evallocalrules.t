...
# Single substitution
# ----------------------------------------------------------------
is( eval_rules( '[{"from":"foo","to":"BAR","opt":"g"}]', "foo and foo and food" ),
    "BAR and BAR and BARd", "global substitution applied" );
...
is( eval_rules(
    ...
    "100 by 200",
    "backslash-style backrefs in replacement work" );

# ----------------------------------------------------------------
# Flags
# ----------------------------------------------------------------
is( eval_rules(
        '[{"from":"hello","to":"hi","opt":"gi"}]',
        "Hello HELLO hello"
    ),
    "hi hi hi",
...
is( eval_rules(
    ...
    "BAR foo foo",
    "empty opt replaces only first occurrence" );

# Unknown flag chars are stripped silently
is( eval_rules(
        '[{"from":"foo","to":"BAR","opt":"gx!@#"}]',
        "foo foo"
    ),
    "BAR BAR",
    "unknown flag chars stripped, valid flags still applied" );

# ----------------------------------------------------------------
# Rule ordering: rules apply sequentially
# ----------------------------------------------------------------
is( eval_rules(
        '[{"from":"a","to":"b","opt":"g"},{"from":"b","to":"c","opt":"g"}]',
        "a"
    ),
    "c",
    "rules apply in order; output of one feeds the next" );
