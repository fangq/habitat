...
# ----------------------------------------------------------------
my @injections =
  ( "Home' OR 1=1--", "Home; DROP TABLE page", "Home\" --", "Home`", "Home\nUNION SELECT", );
...
is( HabitatEngine::ValidId("Home/../Other"), "", "ValidId accepts '..' as a subpage segment" );
...
is( HabitatEngine::FreeToNormal("hello world"),
    "Hello_World", "spaces collapsed, first letter and post-underscore caps" );
is( HabitatEngine::FreeToNormal("Hello_World"), "Hello_World", "already normalized" );
is( HabitatEngine::FreeToNormal("Parent / Child"),
    "Parent/Child", "subpage separator preserved, surrounding spaces dropped" );
is( HabitatEngine::FreeToNormal("Mixed_Case Stuff"),
    "Mixed_Case_Stuff", "mixed case preserved, trailing word capped" );
...
is( HabitatEngine::FreeToNormal("__foo__"),
    "foo", "leading underscores stripped; first letter NOT recapped after strip" );
is( HabitatEngine::FreeToNormal("foo__"),
    "Foo", "trailing-only underscores stripped; ucfirst applied first" );
  ...
  )
{
    ...
    is( HabitatEngine::ValidId($norm), "", "FreeToNormal('$input') = '$norm' passes ValidId" );
}
...
# ----------------------------------------------------------------
{
    ...
    is( HabitatEngine::FreeToNormal("hello world"), "hello_world", "no case fold: spaces only" );
    is( HabitatEngine::FreeToNormal("Mixed_Case Stuff"),
        "Mixed_Case_Stuff", "existing case preserved" );
