...
require DBI;
my $pg = DBI->connect( $pg_dsn, $pg_user, $pg_pass,
...
is( ReadDBItems( "system", "data", "", "", "id=?", "k1" ),
    "v1", "WriteDBItems insert -> ReadDBItems on PG" );
...
is( ReadDBItems( "system", "data,time", "", "|", "id=?", "k1" ),
    "v2|2000", "WriteDBItems upsert (ON CONFLICT DO UPDATE) on PG" );
...
my $r =
  $pg->selectrow_arrayref("SELECT id, revision, author, text FROM page WHERE id='MigratedPage'");
is_deeply(
    $r,
...
my $w = $pg->selectrow_arrayref("SELECT page, username FROM watch WHERE page='MigratedPage'");
is_deeply(
    $w,
    [ "MigratedPage", "alice" ],
