...
time_bench( "ConstantEq (64ch)",   sub {
...
time_bench( "ReadDBItems (single row)", sub {
    Habitat::Store::ReadDBItems( "system", "data", "", "", "id=?", "bench" );
});
time_bench( "WriteDBItems (upsert)", sub {
...
print "DB: in-memory SQLite (Habitat::Store dialect: ",
    Habitat::Store::dialect($dbh), ")\n";
