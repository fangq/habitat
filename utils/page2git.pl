...
foreach $line (@histpage) {
    if ( $line =~
/\([ \*]\)\s*\([ \*]\).*Revision\s+([0-9\.]+)\s*\.\s*\.\s*(\(\w+\))*\s*(.*)\s*by\s*(\[\d+\])*(\w+)/
      )
    {
        $dstr  = `date --date='$3'`;
        $etime = `date --date='$3' +%s`;
...
foreach $etime ( sort keys %revs ) {
    $rev = $revs{$etime}[0];
    qsystem(
"lynx -dont_wrap_pre -dump '$CGI?action=browse&id=$wikipage&revision=$rev&raw=1' > $pgname.wiki"
    );
    qsystem("git add $pgname.wiki");
    qsystem(
"GIT_COMMITTER_DATE='$revs{$etime}[1]' GIT_AUTHOR_DATE='$revs{$etime}[1]' git commit --date '$revs{$etime}[1]' -m '$CGI?action=browse&id=$wikipage&revision=$rev'"
    );
