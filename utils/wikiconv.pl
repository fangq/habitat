...
sub BuildUCScodeNew {
    my ($pgid) = @_;
    if (   $pgid =~ /^\(draft\)UNI_(.*)_([89101234]*)p[tx]/
        || $pgid =~ /^UNI_(.*)_([89101234]*)p[tx]/
        || $pgid =~ /^U([0-9A-Fa-f]+).WQYS/
        || $pgid =~ /^\(draft\)U([0-9A-Fa-f]+).WQYS/ )
    {
        my $ucsbuf = hex($1);
        {
            $UCS2Num = $ucsbuf;
...
sub BuildWikiTree {
    ...
    foreach $dir (@dirs) {
        ...
        if ( -f $fname && $fname =~ /\.db/ ) {
            ...
            if ( $topDir =~ /\/other\/[0-9A-F][0-9A-F]$/ && $lname =~ /^[0-9A-F][0-9A-F]\/(.*)/ ) {
                $lname = $1;
            }
            $treetext .= "$lname\n";
            $fcount++;
...
sub DumpSQL {
    my ($debug) = @_;
    my (
        $page,   $Page,      $Section,  $Text,  @pagelist, %sect,    $bb,
        @revnum, $revision,  $lasttime, %data,  $count,    $oldtext, $rev,
        $dif,    $diffpatch, $content,  $items, @currrev
    );
    ...
    foreach $page (@pagelist) {
        ...
        foreach my $revision (@revnum) {
            ...
            if ( $count > 1 ) {
                ...
                if ( $debug == 1 ) {
                    print "===> $page [rev $revision] time $sect{ts} $rev, $count "
                      . length($content) . "["
                      . join( "/", @currrev ) . "]\n";
                }
                PrintPageSQL( $page, $Page, $KeptRevisions{ $currrev[0] },
                    $content, $rev++, $debug );
                if ( $revision == $revnum[-1]
                    && !( $data{'minor'} == 1 || length($dif) < length($oldtext) * 0.25 ) )
                {
                    @currrev = ($revision);
                    $content = $data{'text'};
                    if ( $debug == 1 ) {
            ...
            if ( @revnum == 1 ) {
                ...
                PrintPageSQL( $page, $Page, $KeptRevisions{$revision},
                    $data{'text'}, $rev++, $debug );
                last;
            }
            $oldtext = $data{'text'};
